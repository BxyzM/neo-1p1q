...
| `circuit_type` | Circuit name from `quantum/circuits/registry.py`: `normal` (production), `experimental_001` (adds a dR-conditioned pairwise readout on top of the ring CNOT entangler), `experimental_002` (like `experimental_001`, but the ring CNOT is replaced entirely by a dR-conditioned IsingZZ gate on every unordered wire pair), or `experimental_003` (like `experimental_002`, but the dR-conditioned gate is IsingYY instead of IsingZZ, and encoding drops the (eta, phi) angle encoding entirely -- each wire gets only a single `RX(scale_factor * pt)` rotation). |
...
| `aux_weights.dr_scale` (`experimental_001`/`experimental_002`/`experimental_003` only) | Starting value for each pair's dR-conditioned entangling-gate angle scale (IsingZZ for `experimental_001`/`experimental_002`, IsingYY for `experimental_003`). Broadcast to one independent trainable value per wire pair used by the circuit (the 10 ring-adjacent pairs for `experimental_001`, all `n*(n-1)/2` unordered pairs for `experimental_002`/`experimental_003`). |
| `aux_weights.pairwise_coeffs` (`experimental_001`/`experimental_002`/`experimental_003` only) | Starting value for each unordered wire pair's `Z_i Z_j` readout coefficient. Broadcast to one independent trainable value per pair (`n*(n-1)/2` total). Unchanged (still Z-basis) even for `experimental_003`, whose entangling gate is IsingYY. |
