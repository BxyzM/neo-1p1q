...
| `circuit_type` | Circuit name from `quantum/circuits/registry.py`: `normal` (production), `experimental_001` (adds a dR-conditioned pairwise readout on top of the ring CNOT entangler), or `experimental_002` (like `experimental_001`, but the ring CNOT is replaced entirely by a dR-conditioned IsingZZ gate on every unordered wire pair). |
...
| `aux_weights.dr_scale` (`experimental_001`/`experimental_002` only) | Starting value for each pair's dR-conditioned IsingZZ angle scale. Broadcast to one independent trainable value per wire pair used by the circuit (the 10 ring-adjacent pairs for `experimental_001`, all `n*(n-1)/2` unordered pairs for `experimental_002`). |
| `aux_weights.pairwise_coeffs` (`experimental_001`/`experimental_002` only) | Starting value for each unordered wire pair's `Z_i Z_j` readout coefficient. Broadcast to one independent trainable value per pair (`n*(n-1)/2` total). |
