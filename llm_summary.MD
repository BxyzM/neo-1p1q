...
- In this environment, `publish_reports.py --results-dir` must always be `/ceph/abal/Neo1P1Q/results` -- every local/condor pipeline script's `dump`/`--results-dir` points there (see e.g. `saved_models/013/run_command.sh`). `publish_reports.py` rebuilds the *entire* report set on every invocation (it iterates all of `--models-dir`, not just one experiment), and a wrong `--results-dir` doesn't error -- it silently reports every experiment not found there as `unavailable`/partial. When pulling a condor job's output back from EOS (`xrdcp`), copy its `results/<experiment_id>` into `/ceph/abal/Neo1P1Q/results/<experiment_id>` (not an ad hoc path) before publishing, for exactly this reason. Ask a different user for their own results path if this one does not apply.

## Current scope

The active path supports two dataset formats and one circuit family:

- Signal versus background classification on JetClass (HDF5) or JetsGame (gzipped JSON lines), selected by `dataset` -- see [Data contract and preprocessing](#data-contract-and-preprocessing).
- Four circuits registered in `quantum/circuits/registry.py`: `normal`, `experimental_001`, `experimental_002`, `experimental_003` -- see [Circuit and score construction](#circuit-and-score-construction) for what each one changes.
...
- `jax==0.10.2` is pinned in `requirements.txt` -- PennyLane 0.45.1's jax interface depends on a `jax.core` API removed in JAX 0.11.
- Analytic execution only (`shots<=0`); `set_circuit()` raises `ValueError` for `backend='jax'` with `shots>0`.
- `QuantumClassifier.__init__` enables `jax.config.update("jax_enable_x64", True)` once, as its first action, only when `backend='jax'` -- JAX defaults to float32 otherwise.
- Training uses `optax.inject_hyperparams(optax.adam)`, with its own checkpoint schema (`optimizer.name == 'optax_adam'`, `opt_state` numpy-converted for portable pickling) validated in `helpers/trained_run.py`. Learning-rate decay mutates `opt_state`'s injected `learning_rate` field directly.
- `QuantumTrainer._jax_train_step`/`_jax_val_step` are each a single `jax.jit`-compiled function, built once per trainer, retraced at most twice per run (regular batch shape, smaller final partial batch).
- `QuantumTrainer.current_weights` is written back from the jax-native `jax_params` on every training call inside `iteration()` -- load-bearing; without it, validation/checkpoints/the certified model silently keep the untrained initial weights.
- `run_experiments.py` and `evaluate.py` disable GPU memory preallocation and pin each jax-backend child to one GPU (see [Configuration behavior](#configuration-behavior)).
- `jax[cuda12_local]` assumes a system CUDA 12.x toolkit already on `LD_LIBRARY_PATH`; see the troubleshooting comment above it in `requirements.txt` if `jax.devices()` reports no `CudaDevice`.
- Not implemented: finite-shot (`shots>0`) execution under `backend='jax'`.
- `QuantumClassifier.set_circuit()` wraps the QNode in `qml.transforms.broadcast_expand` only when `diff_method != 'backprop'` -- `default.qubit` batches natively for backprop, so the transform is needed only for adjoint/finite-shot parameter-shift. Applying it unconditionally splits one batched tape into `batch_size` separate tapes and inflates `jax.jit` compile time by roughly two orders of magnitude.
...
| `configs/base.yaml` | Default configuration for the JetClass VQC. |
| `configs/jetsgame.yaml` | Same schema with the JetsGame dataset, sample names and data path filled in. |
...
| `jetsgame_reader.py` | JetsGame `.json.gz` decoding: four-vectors to jet-relative `(eta, phi, pt)`, hardest-constituent selection, path building. Subclasses `CASEJetClassDataset` and replaces only `_load_file`. |
| `helpers/data.py` | Resolves `dataset` to a file layout and reader. `train.py` and `evaluate.py` locate and build every loader through it. |
...
| `quantum/circuits/vqc.py` | `VQCCircuit`, `VQCExperimental001/002/003` -- see [Circuit and score construction](#circuit-and-score-construction). |
| `quantum/circuits/registry.py` | Maps `circuit_type` (`'normal'`/`'experimental_001'`/`'002'`/`'003'`) to the matching class above. |
...
| `helpers/utils.py` | Feature index constants, per-dataset assumed feature ranges, `sigmoid()`, and general pickle helpers still used by training and evaluation. |
...
```mermaid
flowchart TD
    A[configs/base.yaml or configs/jetsgame.yaml plus key=value overrides] --> B[train.py]
    B --> C[Resolve config and create save_dir/seed/random_seed]
    C --> D[Save config.yaml and circuit snapshot]
    D --> E[Build QuantumClassifier and loaders via helpers/data.py]
    ...
    J --> K[Load the configured dataset's test split]
    K --> L[test_results.pickle and ROC plot]
```

## Data contract and preprocessing

`cfg.dataset` selects the layout and the reader. `helpers/data.py` owns that choice -- `train.py` and `evaluate.py` call `require_files()` once up front (so a missing validation or test split fails before the training split is read) and then `build_loader()` per split, and never touch a reader directly. A configuration without a `dataset` key reads as `jetclass`, so runs saved before the key existed still load.

### JetClass (`dataset: 'jetclass'`, default)
...
### JetsGame (`dataset: 'jetsgame'`)

```text
<data_dir>/
├── train/<sample>_<jetsgame_pt_bin>[_<jetsgame_level>].json.gz
├── valid/valid_<sample>_<jetsgame_pt_bin>[_<jetsgame_level>].json.gz
└── test/test_<sample>_<jetsgame_pt_bin>[_<jetsgame_level>].json.gz
```

Samples are `Top`, `WW`, `QCD`; pt bins `200GeV`, `500GeV`, `2TeV`; levels `truth` (no suffix), `hadron`, `parton`. Note the directory is `valid/`, not `val/`, and that the split prefix is repeated inside the filename. `flat=true` is rejected for this dataset because JetsGame publishes no flattened splits.

Each line is one jet: a JSON list of `{"E","px","py","pz"}` four-momenta in GeV. **Element 0 is the jet itself, not a constituent** -- the remaining elements sum to it exactly, and `JetsGameDataset` verifies that per jet, because mistaking the two would shift every angle and drop the hardest particle. Three things differ from JetClass and are handled in `JetsGameDataset._load_file`:

1. Constituents are `(E, px, py, pz)`, so pt/eta/phi are derived (`eta` is pseudorapidity `arcsinh(pz/pt)`, matching JetClass's convention rather than the rapidity the samples were clustered in).
2. They arrive in **no** pt order, so the hardest `num_particles` are selected with a stable descending sort. The JetClass leading-slice shortcut does not apply.
3. Angles are absolute, so the jet axis is subtracted and delta-phi wrapped into `[-pi, pi)`.

Jets with fewer constituents than `num_particles` are zero-padded at the end, matching JetClass's fixed-width block. Reading is streamed and stops at the requested jet count; roughly 15k jets/s, so a 250k-jet test split costs tens of seconds per class. The files are git-lfs; an un-pulled one is a short text pointer, detected and reported with `git lfs pull` rather than surfacing as a decompression error.

`JetsGameDataset` subclasses `CASEJetClassDataset` and overrides only `_load_file`, `_class_name`, `LOADER_NAME` and `ASSUMED_LIMITS`. Everything else -- scaling, balancing, shuffling, batching -- is the JetClass code path, so a change there affects both datasets.
...
`CASEJetClassDataset` performs the following work when it is constructed (steps 3 and 4 are what `JetsGameDataset` replaces):
...
3. Verifies that particle transverse momenta are in descending order for every loaded jet (tolerance `1e-3` -- real JetClass files can have near-tied constituents a stricter tolerance would reject).
...
- pt: `[1e-4, 3000]` to `[0, 1)`.
- eta and phi: `[-0.8, 0.8]` (JetClass) or `[-1.0, 1.0]` (JetsGame) to approximately `[-pi, pi)`.

The eta/phi window is the jet radius -- JetClass clusters anti-kt R=0.8 jets, JetsGame R=1.0 -- so each reader carries its own range as the class attribute `ASSUMED_LIMITS` (`helpers.utils.assumed_limits` and `jetsgame_assumed_limits`). `fixed_rescale` reads that attribute, not the module global; a new dataset at a different radius sets its own rather than editing the shared one.

Values are scaled but not clipped, so a constituent beyond the assumed window lands outside `[-pi, pi)` and wraps. Measured on 10k JetsGame test jets at `num_particles=10`, that is 0.007% of eta slots and no phi slots -- negligible, but the reason to keep `ASSUMED_LIMITS` matched to the jet radius rather than shared. With `norm_pt=true`, constituent pt is divided by jet pt while eta and phi still use fixed scaling.
...
`summary.total_jets` ("Evaluated jets" on the index card) is the shared per-run test-set size, not
a sum across runs -- every run in an experiment evaluates the identical test jets (`case_reader.py`
selection is deterministic by file/row; `random_seed` only reshuffles order), so summing would
overcount the same jets once per run.
...
The frontend also has a "Parameter sweep" page (`?view=sweep`, `renderSweepPage`/`buildSweepModel`
in `app.js`): among experiments sharing every reported architecture field except one swept
parameter (e.g. `num_layers`), it picks the largest such comparable group and plots mean test AUC
against the swept value, one colored series per `circuit_type` found in that group (`circuit_type`
is never treated as a fixed context field). A multi-select "Circuit type" chip row (an "All" chip
plus one independently toggleable chip per available `circuit_type`, reusing the `.segmented`
CSS pattern) lets the chart/summary be filtered down to a subset of those series; it only hides
already-computed series and never changes which experiments form the comparable group. Selection
is persisted in the `circuits` URL query param and reconciled (kept if still valid, else reset to
"all") whenever the swept parameter changes, since the available circuit types can differ per
parameter. The chip row hides itself when the current group has only one circuit type.
...
- **No memory ceiling on these local jobs.** Only `taskset -c 0-19` (CPU pinning) is applied; there is no `ulimit`/cgroup capping host RAM, which is shared with everyone else on this machine. A silent, traceback-free death of every concurrent training process at the same epoch boundary, with an empty `pipeline.log`, is the signature of an external `SIGKILL` (most likely an OOM-kill) rather than a code bug -- check `logs.log` per seed for how far training actually got before concluding a crash without confirming the process is truly gone (`ps -eo pid,ppid,cmd | grep <path>`, not just `ps -p <remembered PID>`, since a stale/wrong PID can falsely look dead).

Local experiments (`saved_models/001-014`, each with this same `run_command.sh`/`pipeline.log`/`pipeline.pid` pattern) established the working hyperparameter convention for the deeper experimental circuits: `base_2d.yaml` + `circuit_type=experimental_003`, the full 200k/40k/500k split (`flat=false` plus explicit `n_signal`/`n_background`/etc., since the file default is the smaller flat split), `improv=0.001`, `min_epochs=10`, `decay_patience=6`. `num_layers` has been tested locally up to 8; deeper GridKa runs (`num_layers` 16/32/64) are tracked under `condor_experimental_NNN` (see HTCondor Job Submission).
...
- Measurement is the expectation value of a Hamiltonian containing one Pauli Z term per wire. Each wire's coefficient is independently trainable, all starting from `aux_weights.hamiltonian_coeffs` (default 0.1). This requires a differentiation method that supports Hamiltonian/observable coefficients: PennyLane's adjoint method (the default `best` resolution for `lightning.qubit`) cannot differentiate them and silently returns a zero gradient, freezing `hamiltonian_coeffs` at its initial value while `rot`/`scale_factor`/`bias` still train normally. `backprop` (the current default, paired with `default.qubit`) and `parameter-shift` (works on any device/shot count, including `lightning.qubit`, at the cost of roughly two circuit evaluations per differentiable parameter per step) both differentiate `hamiltonian_coeffs` correctly. `adjoint` is the only common option that silently fails for this parameter.

`VQCExperimental001` (subclasses `VQCCircuit`, `circuit_type='experimental_001'`) adds, after the CNOT ring, an `IsingZZ` ring per layer with angle `dr_scale[k] * dR(i,j)` (`dR` computed from the raw eta/phi inputs) on each ring pair, and extends the readout Hamiltonian from `n` Pauli-Z terms to `n` Z_i plus `n*(n-1)/2` Z_i Z_j terms, coefficients `pairwise_coeffs`. A mathematical audit of the base angle-encoding scheme and this circuit's design rationale live in `analysis.md` (gitignored); its supporting test code is under `encoding_test/` and `circuit_test/` (both gitignored).

`VQCExperimental002` (subclasses `VQCExperimental001`, `circuit_type='experimental_002'`) removes the ring CNOT entirely (`entangle()` is a no-op) and instead applies a dR-conditioned `IsingZZ` gate, angle `dr_scale[k] * dR(i,j)`, to every one of the `n*(n-1)/2` unordered wire pairs per layer -- not just the 10 ring-adjacent pairs `VQCExperimental001` uses. Readout is unchanged from `VQCExperimental001` (inherited `measure()`: `n` Z_i plus `n*(n-1)/2` Z_i Z_j terms, coefficients `pairwise_coeffs`). Because `dr_scale` is now broadcast per pair (`n*(n-1)/2` values) rather than per wire, `VQCExperimental002` overrides `aux_per_wire_names`/`aux_per_pair_names` to move `dr_scale` from the former to the latter.

`VQCExperimental003` (subclasses `VQCExperimental002`, `circuit_type='experimental_003'`) makes two further changes on top of `VQCExperimental002`'s all-pairs entangling structure: the dR-conditioned gate is `IsingYY` instead of `IsingZZ` (same angle formula, `dr_scale[k] * dR(i,j)`), and `encode()` drops the (eta, phi) angle encoding entirely -- each wire gets only a single `RX(scale_factor * pt)` rotation (no `RY` term). `dR` is still computed from the raw eta/phi inputs for the entangling gate's angle; only the per-wire encoding rotation changed. Readout, `aux_per_wire_names`, and `aux_per_pair_names` are all inherited unchanged from `VQCExperimental002`.

`CircuitWeights` separates parameters into:

- `rot`, normally shaped `(num_layers, wires, operations_per_qubit)`.
- `aux`, a mapping of named trainable values. `Circuit.aux_per_wire_names` and `Circuit.aux_per_pair_names` list which `aux_defaults` entries `helpers.trained_run.resolve_aux_weights()` broadcasts from one starting scalar to one independent value per wire or per unordered wire pair, respectively (`VQCCircuit`: `hamiltonian_coeffs` per wire; `VQCExperimental001` adds `dr_scale` per wire and `pairwise_coeffs` per pair; `VQCExperimental002` keeps `hamiltonian_coeffs` per wire but moves `dr_scale` to per pair alongside `pairwise_coeffs`). A circuit's `measure()` should raise a clear error naming the missing key if an aux weight it needs is absent (e.g. resuming/evaluating a checkpoint trained with a different `circuit_type`), rather than a bare `KeyError`.
...
The `finally` block runs even after an uncaught exception in the training loop (e.g. a CUDA OOM
crash mid-epoch): a `history.pickle` stub and a Weights & Biases run showing `state: finished` are
both produced regardless of whether training actually completed. Neither is evidence of success --
confirm via logged epoch count / `val_auc` / `train_loss` in the WandB history (a crashed run has
zero logged epochs and near-random `val_auc`).
...
jax-backend tests are guarded with `skipUnless(_HAS_JAX, ...)` and run whenever jax is installed (true in `pennylane-gpu-jax-sep2026`). JetClass integration tests are separately guarded with `skipUnless` and run only when `/ceph/abal/JetClass/test/TTBar_` and `/ceph/abal/JetClass/test/ZJetsToNuNu` exist (true in this environment). The JetsGame equivalent is guarded on `/ceph/mbinder/JetGame/data-v1.0.0/test/test_{Top,QCD}_500GeV.json.gz` (the directory on disk is spelled `JetGame`; the dataset is JetsGame) and overridable with `JETSGAME_DIR`; the rest of `test_jetsgame_reader.py` builds its own fixtures and needs no shared storage.
...
| `test_jetsgame_reader.py` | JetsGame path building, four-vector decoding against known kinematics, hardest-constituent selection, delta-phi wrapping, zero padding, both scaling paths, git-lfs/non-gzip/missing-jet-header rejection, balancing and seeded shuffling, `helpers.data` dispatch, and optional real JetsGame loading. |
...
- Data passed to the circuit has shape `(batch, num_particles, 3)`, in `(eta, phi, pt)` order, whichever dataset produced it.
...
To add a dataset format:

1. Subclass `CASEJetClassDataset` and override `_load_file` to return `(M, num_particles, 3)` already scaled; override `ASSUMED_LIMITS` if the jets have a different radius, and `LOADER_NAME`/`_class_name` for readable logs. Reuse the inherited balancing, shuffling and batching rather than reimplementing them.
2. Add the name to `helpers.config.SUPPORTED_DATASETS`, and reject the options that do not apply to it (as `flat` is for JetsGame) instead of ignoring them.
3. Teach `helpers/data.py`'s `split_label`, `class_files` and `build_loader` about the layout. Do not branch on the dataset in `train.py` or `evaluate.py`.
4. Add the reportable config keys to `publish_reports._config_groups`.
5. Add decoding tests with self-built fixtures, plus a `skipUnless`-guarded test against the real files.
...
2. Declare `operations_per_qubit` and `aux_defaults` (plus `aux_per_wire_names`/`aux_per_pair_names` for any aux weight that needs one independent value per wire or per unordered wire pair).
...
`condor_test/`, `condor_submission/`, and its `separate_jobs/`/`concurrent_jobs/` subdirectories are
gitignored, local-only. Submit node is `deepthought2.etp.kit.edu` (not a worker); the GPU+Docker pool
is a separate cluster, GridKa/TOpAS, invisible to a plain `condor_status`/`condor_q -better-analyze`
on this submit node (always reports "no match" regardless of real matchability). Query GridKa
directly: `condor_status -pool c03-001-140.gridka.de -constraint 'PartitionableSlot &&
TotalSlotGPUs>0' -af:t Machine GPUs_DeviceName Cpus Memory GPUs`, deduplicated by `Machine` (each
host reports several identical rows -- redundant partitionable-slot definitions, not one row per
extra GPU). The pool mixes `Tesla V100S-PCIE-32GB` and `NVIDIA A100-PCIE-40GB` nodes.
`condor_userprio -pool c03-001-140.gridka.de -allusers` shows this account's fairshare priority under
`accounting_group = cms.higgs`. `condor_history` can hang on this schedd's large history file --
prefer `condor_q -af ...`.

Reaching GridKa needs `+RemoteJob = True` and a valid proxy at `/home/abal/.globus/x509up` (check
with `voms-proxy-info -all`, don't assume a path). With those plus `requirements =
regexp("A100|V100", TARGET.GPUs_DeviceName)` (or `"A100"`/`"V100"` alone to target one type), jobs
match reliably once a slot with free resources actually accepts this account (see the troubleshooting
notes below) -- multi-GPU requests (`request_GPUs = 2`) work fine and are sometimes necessary (see
`condor_experimental_NNN` below), contrary to earlier notes here claiming otherwise.

### GPU pool troubleshooting

`condor_gpu_analysis` (added to `~/.bashrc`) prints free/total GPUs per node, split by V100/A100,
deduplicated by machine. Two things it won't tell you, both learned the hard way:

- **Per-node owner allow-lists.** Some GridKa GPU nodes restrict `Start` to a fixed list of
  usernames regardless of free resources (e.g. a node with 4/8 GPUs genuinely free still refuses
  every job from an account not on its list). Check with
  `condor_status -pool c03-001-140.gridka.de -constraint 'Machine=="<host>.gridka.de" &&
  PartitionableSlot' -l | grep -m1 '^Start ='` -- a bare boolean expression naming specific
  `Target.Owner` values (rather than a resource-based formula) means that node is restricted; ask
  an admin to add the account rather than assuming a resource-sizing problem.
- **V100 `GPUMemoryMB` is a flat per-node total, not scaled per GPU.** These nodes have 8 real,
  separate physical GPUs (confirmed via distinct `DeviceUuid`/`DevicePciBusId` per slot, `Capability
  = 7.0` -- Volta has no MIG/partitioning, so 8 slots really is 8 cards), but
  `TotalSlotGPUMemoryMB` is declared as one GPU's worth (~32000) for the whole node instead of
  `8x32000`. HTCondor's schedd-side default (`RequestGPUMemoryMB = TARGET.TotalSlotGPUMemoryMB`)
  means the *first* job to claim any GPU on such a node consumes the entire declared pool,
  starving the other 7 idle GPUs until that job finishes/drains. Confirmed by two different users'
  1-GPU jobs each showing `GPUMemoryMB=32000` consumed while sibling dynamic slots showed
  `GPUMemoryMB=0` free with `GPUs=7` still idle. Workaround: add `+RequestGPUMemoryMB = 1` (or any
  small value) to `submit.sub` -- this only affects the memory-accounting bucket, not which
  physical GPU gets assigned (that's the separate, correctly-behaving `GPUs`/device resource), so
  it's safe. A100 nodes don't define `GPUMemoryMB` at all (`undefined`), so this bug is V100-only.
- **GPU VRAM exhaustion is separate from `RequestMemory`.** `RequestMemory` only governs host RAM.
  `concurrent_jobs`-style designs run 5 concurrent training processes sharing whatever GPUs the job
  was granted (`--gpu-id` round-robins them via `CUDA_VISIBLE_DEVICES`); at deep `num_layers`, 5
  processes sharing one 40GB A100 can exceed its VRAM (`CUDA_ERROR_OUT_OF_MEMORY` /
  `jax.errors.JaxRuntimeError: RESOURCE_EXHAUSTED`) regardless of how much host RAM was requested.
  Mitigate by requesting more GPUs (`request_GPUs = 2`+) and passing the matching comma-separated
  list to both `run_experiments.py --gpu-id` and `evaluate.py --gpu-id`, not by raising
  `RequestMemory` -- but GPU count must scale with the run count, not just be `>1`: at
  `num_layers=64`, 5 runs split across 2 GPUs still OOM'd 3/5 (the 3 sharing one GPU), only 2/5
  actually trained. Verify success per run from WandB metrics, not from `run_experiments.py`
  finishing without a crash -- see the `finally`-block note in Training flow.

Data staging: `condor_submission/copy-eos.sh` stages the fixed 10-file JetClass subset to CERN EOS
(`/eos/user/a/aritra/JetClass/...`) via `xrdcp`. Inside a job, `executable.sh` pulls the same files
back from EOS via the CVMFS Belle2-DIRAC `xrdcp`
(`BELLE2_EXEC=/cvmfs/belle.kek.jp/grid/diracos2/2.42/Linux-x86_64/diracos/bin`, auto-mounted), with
`X509_CERT_DIR`/`X509_VOMS_DIR`/`X509_VOMSES` exported to
`/cvmfs/grid.cern.ch/etc/grid-security/...` (this repo's `docker/Dockerfile` doesn't bake these).
Output (`saved_models/`, `results/`, never the staged `data/`) is pushed back to EOS the same way
(`xrdfs ... mkdir -p` + `xrdcp -rf`, which creates the full remote tree from scratch). `submit.sub`
sets `transfer_output_files = ""` since output is handled explicitly via EOS -- HTCondor's own
`transfer_output_remaps` cannot remap directories, only individual files, so it isn't usable for a
whole run's output tree.

Every job design shares one `repo_zipper.sh`-built `Neo1P1Q.tar.gz`
(`train.py`/`run_experiments.py`/`evaluate.py`/`case_reader.py`/`helpers/`/`quantum/` +
`configs/condor_config.yaml` + `configs/base_2d.yaml`). `configs/condor_config.yaml`
(`circuit_type='normal'`, `flat: false`, 200k/40k/500k train/val/test jets -- every non-flat
JetClass file holds exactly 100,000 jets, so this needs 1 file/class train, 1 whole file/class
val, 3 files/class test) and `configs/base_2d.yaml` (gitignored, local-only; `circuit_type=
'experimental_001'`, `dr_scale`/`pairwise_coeffs` aux defaults added) differ in **two** fields, not
one: circuit *and* `flat` (`base_2d.yaml` defaults `flat: true`, small 10k/2k/20k split). **Gotcha:**
`copy-eos.sh` only ever stages the non-flat `train/val/test` layout to EOS, never `flat_train`/
`flat_val`/`flat_test` -- any `base_2d.yaml`-based condor job must explicitly override
`flat=false` (plus `n_signal`/`n_background`/etc. if the larger split is wanted) or every run
crashes with `FileNotFoundError` for the flat splits (hit `condor_concurrent_010` and an early
`condor_experimental_NNN` attempt both this session). Rebuild the tarball (`bash
condor_submission/repo_zipper.sh`) after any code or config change, before submitting.

- **`condor_submission/concurrent_jobs/condor_concurrent_NNN/`** (current default): one job runs
  `run_experiments.py --number-of-runs 5 --num-processes 5 --gpu-id 0` (5 concurrent runs sharing 1
  GPU), then `evaluate.py --experiment-dir`, which aggregates all 5 automatically
  (`roc_curve_summary.png`, `evaluation_summary.log`). Each experiment is self-contained
  (`executable.sh`/`submit.sub`, `EXPERIMENT_SEED` set per directory; config/CLI overrides vary per
  experiment -- 001-009 sweep `num_layers`/`improv` on `condor_config.yaml`, 010 switches to
  `base_2d.yaml`/`circuit_type='experimental_001'` but never overrode `flat` and crashed every run
  with `FileNotFoundError` for `flat_train`/`flat_val` -- see the `flat=false` gotcha below). Sized
  per-experiment at 8 CPUs / 1 GPU / 4h, `RequestMemory` tuned individually (16000-64000, higher for
  deeper circuits or untested designs). `concurrent_jobs/copy_back_and_publish.sh <experiment_name>`
  copies the whole `saved_models/<experiment>` + `results/<experiment>` EOS subtree back and
  republishes the interactive report.
- **`condor_submission/concurrent_jobs/condor_experimental_NNN/`** (newer series, same
  `run_experiments.py`+`evaluate.py` design as `condor_concurrent_NNN`): sweeps `num_layers` for
  `circuit_type='experimental_003'` on `base_2d.yaml` at the full 200k/40k/500k split (matching
  local `saved_models/011-014`'s config exactly -- `n_signal`/`n_background`/etc. and `flat=false`
  must all be passed explicitly, since `base_2d.yaml` alone defaults to the smaller flat=true
  split). `requirements` targets `A100` only (owner-restricted/occupied A100 nodes and a stuck
  V100 drain made straight `A100|V100` unreliable this round -- see GPU pool troubleshooting
  above). 001/002/003 = `num_layers` 8/16/32 at 8-10 CPUs and 40-112GB RAM; 001 was later redone at
  `num_layers=64` with `request_GPUs=2` (`--gpu-id 0,1`). Final outcomes: 002 (`num_layers=16`)
  succeeded fully, 5/5 runs, published. 003 (`num_layers=32`, 1 GPU) and the redone 001
  (`num_layers=64`, 2 GPUs) both still failed -- only 1/5 and 2/5 runs respectively actually
  trained (the rest crashed with `CUDA_ERROR_OUT_OF_MEMORY` at epoch 1, batch 0; WandB showed all
  10 runs as `state: finished` regardless, see the `finally`-block note in Training flow), and
  since `run_experiments.py` itself exits 1 whenever any run fails, `evaluate.py` never ran for
  either job and nothing reached EOS -- the successfully-trained runs' models were never copied out
  and are unrecoverable. See the GPU VRAM bullet above.
- **`condor_submission/separate_jobs/`** (alternative to `concurrent_jobs/`): one `train.py` +
  single-run `evaluate.py --seed --random-seed --model-dir` call per job instead of
  `run_experiments.py`, each needing only 1 GPU. Per job, `RANDOM_SEED` is the sole executable
  argument (`arguments = $(seed)`, `queue seed from seeds.txt`); the experiment name
  (`EXPERIMENT_SEED`, i.e. config `seed=`) is fixed inline in that pair's `executable.sh`/header
  comments, not queued. Sized at 2 CPUs / 16GB RequestMemory / 1 GPU / 6h walltime per job.
  `generate_seeds.sh` writes fresh random seeds to `seeds.txt` per submission; `submit_jobs.sh`
  regenerates seeds then runs `condor_submit`.
  - Each new experiment gets its own `condor_00N/` subdirectory (mirroring `concurrent_jobs/`): copy
    `executable.sh`/`submit.sub`/`generate_seeds.sh`/`submit_jobs.sh` in and update
    `EXPERIMENT_SEED`/header comments/`submit.sub`'s title to the new name; running
    `submit_jobs.sh` from inside that directory generates its own `seeds.txt` and writes `logs/`
    relative to it. `condor_003/` currently exists in this form but is unsubmitted (no `seeds.txt`,
    empty `logs/`).
  - `condor_002` (the first separate-jobs experiment, completed) predates this per-directory
    convention: its `executable.sh`/`submit.sub`/`generate_seeds.sh`/`submit_jobs.sh`/`seeds.txt`
    sit directly at `separate_jobs/`'s top level instead of in a subdirectory of their own.
  - `separate_jobs/copy_back_and_publish.sh <experiment_name> [seeds_file]` is shared (not
    duplicated per subdirectory) and copies each seed's own leaf directory back individually;
    `seeds_file` defaults to `seeds.txt` next to the script, so a subdirectory-style experiment
    must pass its own path explicitly (e.g. `copy_back_and_publish.sh condor_003
    condor_003/seeds.txt`).
- **`condor_submission/executable.sh`/`submit.sub`** (top level): the original `run_experiments.py`
  design that `concurrent_jobs/` now supersedes; kept as-is.

WandB: `docker/Dockerfile` bakes `WANDB_MODE=offline`; every design's `executable.sh` overrides this
with `export WANDB_MODE=online` (`train.py`'s `wandb.init()` has no `mode=` kwarg, purely
env-var-driven) -- needs `WANDB_API_KEY` sourced from `.env` into the shell that runs
`condor_submit` (`getenv = WANDB_API_KEY` only inherits what that shell already has). Clean up
crashed/failed runs via `wandb.Api()`'s `Run.delete()`, matched by name prefix and confirmed
`state`, not guessed.
